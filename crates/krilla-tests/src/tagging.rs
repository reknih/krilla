...
use krilla::tagging::tag::{ListNumbering, TableCellSpan, TableHeaderScope, Tag, TagId, FigureTag, HnTag, LTag, TDTag, THTag, TableTag, PTag, SectionTag, NoteTag, THeadTag, TBodyTag, TFootTag, TRTag, LinkTag};
...
fn tagging_image_with_alt(document: &mut Document) {
    let mut tag_tree = TagTree::new();
    let mut image_group =
        TagGroup::new(FigureTag::new().with_alt_text("This is the alternate text.".to_string()));
...
fn tagging_multiple_pages(document: &mut Document) {
    ...
    let mut heading_1 = TagGroup::new(
        HnTag::new(NonZeroU32::new(1).unwrap()).with_title("first heading".into()),
    );
    let mut heading_2 = TagGroup::new(
        HnTag::new(NonZeroU32::new(1).unwrap()).with_title("second heading".into()),
    );
...
fn tagging_heading_level_7_and_8_impl(document: &mut Document) {
    ...
    let mut new_heading = |level, name| {
        ...
        let mut heading = TagGroup::new(HnTag::new(level).with_title(name.into()));
        heading.push(hn);
...
fn tagging_table_header_and_footer(document: &mut Document) {
    ...
    let header = {
        ...
        for x in 0..3 {
            ...
            let tag = THTag::new(TableHeaderScope::Column).with_id(header_id(x));
    ...
    for y in 1..4 {
        ...
        for x in 0..3 {
            ...
            let tag = TDTag::new().with_headers(headers.into());
            row.push(TagGroup::with_children(tag, vec![Node::Leaf(text)]));
        }
        body.push(row);
    }
    ...
    let footer = {
        ...
        let cell = TDTag::new()
            ...
            .with_headers((0..3).map(header_id).collect());
    ...
    let mut table = TagGroup::new(TableTag::new().with_summary("table summary".into()));
    table.push(header);
    table.push(body);
    table.push(footer);
...
fn tagging_id_appears_twice() {
    ...
    let loc_1 = 1;
    let loc_2 = 2;
    let group_1 = TagGroup::new(PTag::new().with_id(id.clone()).with_location(loc_1));
    let group_2 = TagGroup::new(PTag::new().with_id(id.clone()).with_location(loc_2));
